  ...
  // QR code for donations
    ...
    if (donateBtn && donationModal) {
      ...
      donateBtn.addEventListener('click', function() {
        const authorName = post.author && post.author.name ? post.author.name : 'Debug User';
        modalTitle.textContent = `Donate to ${authorName}`;
        
        // Set QR code if available
        const modalQrCode = donationModal.querySelector('.qr-code');
        if (post.qrCodeUrl && modalQrCode) {
          modalQrCode.src = post.qrCodeUrl;
