...
// right after you detect you're on posts.html:
if (window.location.pathname.includes('posts.html')) {
  const authRequired = document.getElementById('auth-required');
  const postsContent = document.getElementById('posts-content');
  
  // Add this line - define isAuthenticated using your existing function
  const isAuthenticated = checkAuthentication(); 

  if (!isAuthenticated) {
    // still not logged in → show the "please log in" message
    authRequired.style.display = 'flex';
    postsContent.style.display = 'none';
  } else {
    // logged in → hide the auth‐message & reveal your posts UI
    authRequired.style.display = 'none';
    postsContent.style.display = 'block';

    // *only now* fetch & render
    loadPosts();
  }
}
...
function initializePostsPage() {
  ...
  const storyForm = document.getElementById('post-form'); // Define storyForm first
  ...
  if (posts.length === 0) {
    ...
    }); // Missing closing bracket for forEach
  ...
  } // Missing closing bracket for if(storyForm)
} // Missing closing bracket for initializePostsPage function
