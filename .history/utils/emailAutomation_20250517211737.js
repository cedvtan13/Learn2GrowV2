 ...
 */
export const processPendingEmails = async (force = false) => {
  try {
    console.log(`🔄 Starting batch processing of${force ? ' ALL' : ' pending'} emails`);
    ...
    if (force) {
      // If force flag is true, get all requests regardless of email status
      console.log('⚠️ FORCE MODE: Processing all recipient requests regardless of email status');
      requests = await RecipientRequest.find({});
    } else {
      // Normal mode: Get all requests that might need emails
      requests = await RecipientRequest.find({
        $or: [
    ...
    for (const request of requests) {
      try {
        ...
        // Send verification emails for rejected requests
        if (request.status === 'rejected' && !request.emailsSent?.verification) {
