...
const createTransporter = () => {
  ...
  const EMAIL_PASS = process.env.EMAIL_PASS || 'kwdhumbjzlqitdxg'; // Hardcoded as fallback
  ...
  if (!config.auth.user || !config.auth.pass) {
    ...
    console.error('Please check your .env file and make sure EMAIL_USER and EMAIL_PASS are set.');
  }
  ...
  console.log({
    ...
    auth: { 
      ...
      pass: config.auth.pass ? '****' : 'MISSING PASSWORD!'
    }
  });
...
const NODE_ENV = 'production'; // Hard-coded to ensure emails are sent
const isDevelopment = false;   // Hard-coded to ensure emails are sent
console.log(`📧 Email mode: Production (sending)`);
console.log(`📧 NODE_ENV: ${NODE_ENV} (forced to production)`);
