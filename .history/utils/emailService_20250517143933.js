...
/**
 * Send a rejection email to the user whose recipient status was rejected
 * @param {string} name - The name of the recipient
 * @param {string} email - The email of the recipient
 * @param {string} notes - Optional notes explaining the rejection reason
 */
export const sendRecipientRejection = async (name, email, notes = '') => {
  try {
    const mailOptions = {
      from: `"Learn2Grow" <${process.env.EMAIL_USER || 'noreply@learn2grow.org'}>`,
      to: email,
      subject: 'Your Learn2Grow Application Status',
      html: `
        <h2>Dear ${name},</h2>
        ...
        <p>If you believe there has been an error or would like more information about this decision, 
           please contact our support team at <a href="mailto:support@learn2grow.org">support@learn2grow.org</a>.</p>
    ...
    if (isDevelopment) {
      console.log('📧 Development Mode - Rejection email would be sent to user:');
      console.log(mailOptions);
      return true;
    } else {
      const info = await transporter.sendMail(mailOptions);
