...
/**
 * Process verification emails for all pending recipient requests
 * @param {Boolean} forceResend - Whether to resend emails even if already marked as sent
 * @returns {Object} Information about emails sent and failed
 */
export const processAllVerificationEmails = async (forceResend = false) => {
  try {
    ...
    // Only include those that haven't had verification emails if not forcing resend
    if (!forceResend) {
      query['emailsSent.verification'] = { $ne: true };
    }
    
    const requests = await RecipientRequest.find(query);
    console.log(`📋 Found ${requests.length} recipients that need verification emails`);
    ...
    for (const request of requests) {
      try {
        const verificationSent = await processVerificationEmail(
          request, 
    ...
    console.log(`✅ Verification email processing completed: ${results.sent} sent, ${results.failed} failed`);
    return results;
  } catch (error) {
    console.error('❌ Error in processAllVerificationEmails:', error);
    return { total: 0, sent: 0, failed: 0, error: true, message: error.message };
  }
};
