...
router.post('/request-recipient', async (req, res) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash(password, salt);
      ...
      console.log(`📧 Sending emails for new recipient: ${name} (${email})`);
      ...
      try {
        ...
        // Save the updated recipient request to persist email status
        await request.save();
        ...
        // Double-check the email status in database
        const updatedRequest = await RecipientRequest.findById(request._id);
        console.log(`📊 Email status in database: ${JSON.stringify(updatedRequest.emailsSent)}`);
      } catch (emailError) {
        console.error(`❌ Error sending emails to ${email}:`, emailError);
