...
router.post('/', async (req, res) => {
  try {
    ...
    if (req.user.role !== 'Recipient' && req.user.role !== 'Admin') {
      return res.status(403).json({ message: 'Only Recipients can create posts.' });
    }
    ...
    try {
      // Try to populate the author, but don't fail if it doesn't work
      await post.populate('author', 'name email');
    } catch (populateError) {
      console.warn('Could not populate author:', populateError);
      // Add a placeholder name so the frontend doesn't crash
      post._doc.author = { 
        _id: authorObjectId,
        name: 'Debug User'
      };
