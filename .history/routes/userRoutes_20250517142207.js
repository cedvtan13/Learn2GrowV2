...
router.post('/request-recipient', async (req, res) => {
  try {
    ...
    // Store the request in a temporary database or send an email to admin
    // For this implementation, we'll just log the request details and send a success response
    console.log(`🔔 New Recipient registration request received: ${name} (${email})`);

    // In a production environment, you'd send an email to the admin here
    // sendEmailToAdmin(name, email);
