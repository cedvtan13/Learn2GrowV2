...
router.post('/request-recipient', async (req, res) => {
  try {
    ...
    try {
      ...
      // Send emails
      const adminEmailSent = await emailService.sendRecipientRequestToAdmin(name, email);
      const confirmationSent = await emailService.sendRecipientConfirmation(name, email);
      
      // Update email tracking in the database
      if (confirmationSent) {
      ...
      console.log(`✉️ Notification emails sent for ${name} (${email}): Admin notification: ${adminEmailSent}, Confirmation: ${confirmationSent}`);
    } catch (storageError) {
  ...
  try {
    ...
    if (status === 'approved') {
      ...
      try {
        const emailServiceModule = await import('../utils/emailService.js');
        const emailService = emailServiceModule.default;
        const emailSent = await emailService.sendRecipientApproval(request.name, request.email);
        
        // Update email tracking in the database
        if (!request.emailsSent) {
          request.emailsSent = {};
        }
        request.emailsSent.approval = emailSent;
        if (emailSent) {
          request.lastEmailSent = new Date();
        }
      } catch (emailError) {
    ...
    } else if (status === 'rejected') {      // Send verification email instead of rejection
      try {
        const emailServiceModule = await import('../utils/emailService.js');
        const emailService = emailServiceModule.default;
        const emailSent = await emailService.sendRecipientVerification(request.name, request.email, notes);
        
        // Update email tracking in the database
        if (!request.emailsSent) {
          request.emailsSent = {};
        }
        request.emailsSent.verification = emailSent;
        if (emailSent) {
          request.lastEmailSent = new Date();
        }
        
        console.log(`📧 Verification email sent to ${request.email}: ${emailSent ? 'Success' : 'Failed'}`);
      } catch (emailError) {
  ...
  try {
    ...
    switch(emailType) {
      ...
      case 'approval':
        ...
        break;
      case 'rejection':
        result = await emailService.sendRecipientRejection(testName, testEmail, testNotes);
        break;
      default:
        return res.status(400).json({ message: 'Invalid email type. Use "request", "approval", or "rejection".' });
    }
